...
public interface HCNetSDK extends Library {
	...
	String strDllPath = System.getProperty("user.dir") + "\\lib\\HCNetSDK\\HCNetSDK.dll";
	HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(strDllPath, HCNetSDK.class);
...
//矩阵行信息 2007-12-28
	...
	public static class NET_DVR_SCENE_CONTROL_INFO extends Structure {
		...
		public int dwCmd; // 场景控制命令，1-场景模式切换（如果要切换的是当前场景，则不进行切换），2-初始化场景（将此场景的配置清空，如果是当前场景，则同时对当前场景进行清屏操作），3-强制切换（无论是否是当前场景，强制切换），4-保存当前模式到某场景
							// 5-删除场景 ,6-场景复制
	...
	public static class NET_DVR_PLAN_CFG extends Structure {
		...
		public NET_DVR_CYCLE_TIME[] struTimeCycle = new NET_DVR_CYCLE_TIME[DAYS_A_WEEK/* 7 */]; /*
																								 * 循环时间，周期为一个星期，年、月、
																								 * 日三个参数不使用。如：
																								 * struTimeCycle[0]
																								 * 中的byValid的值是1，
																								 * 表示星期天执行该预案。星期取值区间为[0,
																								 * 6]，其中0代表星期天，1代表星期一，
																								 * 以此类推
																								 */
	...
	public static class NET_DVR_PTZABSOLUTEEX_CFG extends Structure {
		...
		/*
		 * 镜头变倍配置类型；absoluteZoom:通过变倍参数进行配置，选择为该类型时struPTZCtrl中的fZoom参数生效。focalLen:
		 * 通过焦距参数进行配置，选择为该类型时，dwFocalLen参数生效。
		 */
...
//NET_DVR_Login()参数结构
	...
	public static class NET_DVR_FILECOND_V50 extends Structure {
		...
		public int dwTimeout; // 查找超时时间（指定NET_DVR_FindNextFile_V30/NET_DVR_FindNextFile_V40/NET_DVR_FindNextFile_V50接口的超时时间返回）；单位：毫秒，不填写（默认为0时），接口行为跟以前一样
								// ;有效值：0, [5000 – 15000]
	...
	public static class NET_DVR_PACKET_INFO_EX extends Union {
		...
		public int dwPacketType; // Packet type:0 -file head，1 -video I frame，2- video B frame， 3- video P frame，
									// 10- audio packet， 11- private packet
	...
	public static class NET_DVR_SETUPALARM_PARAM_V50 extends Structure {
		...
		// Bit6-
		// 表示雷达检测报警(eventType:radarDetection)是否开启实时上传；0-不开启，1-开启（用于web插件实时显示雷达目标轨迹）
		public byte bySupport;
		// 断网续传类型
		...
		// bit3 - 行为分析(报警类型为COMM_ALARM_RULE)中图片数据上传类型：0 - 二进制传输，1 -
		// URL传输，本字段设备是否支持，对应软硬件能力集中<isSupportBehaviorUploadByCloudStorageURL>节点是否返回且为true
	...
	public static class NET_VCA_RUN extends Structure {
		...
		/*
		 * 检测目标，可支持多选，具体定义为： 0~所有目标（表示不锁定检测目标，所有目标都将进行检测） 0x01 ~ 人， 0x02 ~ 车，， 0x04 ~
		 * 其他， 该字段支持多选，按位取值，例如3表示1+2.
		 */
		public byte byDetectionTarget;
		public byte byRes;
	}
	...
	public static class NET_DVR_SENSOR_ALARM extends Structure {
		...
		public byte byAlarmMode; // 报警模式，五种，-HHHH、-HHHL、-HHLL、HLLL、-LLLL，
									// 作为平台报警程度判断功能,即：1111（上上上上），1110（上上上下），1100（上上下下），1000(上下下下)，0000(下下下下)
	...
	public static class NET_DVR_TRAFFIC_DATA_QUERY_COND extends Structure {
		...
		/*
		 * Bit0-未ª知（其他） Bit1-白色 Bit2-银色 Bit3-灰色 Bit4-黑色 Bit5-红色 Bit6-深蓝色 Bit7-蓝色 Bit8-黄色
		 ...
		 * Bit8-禁止名单 Bit9-超速 Bit10-变道 Bit11-掉头 Bit12-机占非 Bit13-加塞
		 */
		public int dwEventType;
		/**
	...
	public static class NET_DVR_CARD_READER_CFG_V50 extends Structure {
		...
		public byte byCardReaderType; // 读卡器类型，1-DS-K110XM/MK/C/CK，2-DS-K192AM/AMP，3-DS-K192BM/BMP，4-DS-K182AM/AMP，5-DS-K182BM/BMP，6-DS-K182AMF/ACF，7-韦根或485不在线，8-
										// DS-K1101M/MK，9- DS-K1101C/CK，10- DS-K1102M/MK/M-A，11- DS-K1102C/CK，12-
										// DS-K1103M/MK，13- DS-K1103C/CK，14- DS-K1104M/MK，15- DS-K1104C/CK，16-
										// DS-K1102S/SK/S-A，17- DS-K1102G/GK，18- DS-K1100S-B，19- DS-K1102EM/EMK，20-
										// DS-K1102E/EK，21- DS-K1200EF，22- DS-K1200MF，23- DS-K1200CF，24- DS-K1300EF，25-
										// DS-K1300MF，26- DS-K1300CF，27- DS-K1105E，28- DS-K1105M，29- DS-K1105C，30-
										// DS-K182AMF，31-
										// DS-K196AMF，32-DS-K194AMP，33-DS-K1T200EF/EF-C/MF/MF-C/CF/CF-C,34-DS-K1T300EF/EF-C/MF/MF-C/CF/CF-C，35-DS-K1T105E/E-C/M/M-C/C/C-C,36-DS-K1T803F/F-M/F-S/F-E,37-DS-K1A801F/F-M/F-S/F-E,38-DS-K1107M/MK,39-DS-K1107E/EK,40-DS-K1107S/SK,41-DS-K1108M/MK,42-DS-K1108E/EK,43-DS-K1108S/SK,44-DS-K1200F,45-DS-K1S110-I,46-DS-K1T200M-PG/PGC,47-DS-K1T200M-PZ/PZC,48-DS-K1109H
		...
		public byte byFingerPrintCheckLevel; // 指纹识别等级，1-1/10误认率，2-1/100误认率，3-1/1000误认率，4-1/10000误认率，5-1/100000误认率，6-1/1000000误认率，7-1/10000000误认率，8-1/100000000误认率，9-3/100误认率，10-3/1000误认率，11-3/10000误认率，12-3/100000误认率，13-3/1000000误认率，14-3/10000000误认率，15-3/100000000误认率，16-Automatic
												// Normal,17-Automatic Secure,18-Automatic More Secure（目前门禁不支持）
		public byte byUseLocalController; // 只读，是否连接在就地控制器上，0-否，1-是
		public byte byRes1;
	...
	public static class NET_DVR_MULTI_CARD_CFG_V50 extends Structure {
		...
		public byte[] byRes1 = new byte[2];
		public NET_DVR_MULTI_CARD_GROUP_CFG_V50[] struGroupCfg = (NET_DVR_MULTI_CARD_GROUP_CFG_V50[]) new NET_DVR_MULTI_CARD_GROUP_CFG_V50()
				.toArray(20);
		public byte[] byRes2 = new byte[32];
	}
	...
	public static class NET_DVR_MULTI_CARD_GROUP_CFG_V50 extends Structure {
		...
		public NET_DVR_GROUP_COMBINATION_INFO_V50[] struGroupCombination = (NET_DVR_GROUP_COMBINATION_INFO_V50[]) new NET_DVR_GROUP_COMBINATION_INFO_V50()
				.toArray(8);
	}
	...
	public static class NET_DVR_SEARCH_EVENT_PARAM_V50 extends Structure {
		public short wMajorType; // 0-移动侦测，1-报警输入, 2-智能事件 5-pos录像 7-门禁事件, 8-非视频联动事件
		public short wMinorType; // 搜索次类型- 根据主类型变化，0xffff表示全部
		public NET_DVR_TIME_SEARCH_COND struStartTime = new NET_DVR_TIME_SEARCH_COND(); // 搜索的开始时间，停止时间: 同时为(0, 0)
																						// 表示从最早的时间开始，到最后，最前面的4000个事件
	...
	public static class EVENT_IOTPARAM_V50 extends Structure {
		public short wDeviceType; // 设备类型,0-海康门禁主机，1-海康可视对讲设备, 2-海康报警主机（预留） 3-GJD报警主机 4-Luminite报警主机,
	...
	public static class EVENT_STREAMIDRET_V50 extends Structure {
		public int dwRecordType; // 录像类型 0-定时录像 1-移动侦测 2-报警录像 3-报警|移动侦测 4-报警&移动侦测 5-命令触发 6-手动录像 7-震动报警 8-环境触发
									// 9-智能报警 10-回传录像
	...
	public static class NET_DVR_ALARM_SEARCH_COND extends Structure {
		...
		/*
		 * 报警命令，该字段值与报警布防类型相同，目前支持： COMM_VCA_ALARM 0x4993 智能检测报警
		 * COMM_UPLOAD_FACESNAP_RESULT 0x1112 人脸识别结果上传 COMM_SNAP_MATCH_ALAR 0x2902
		 * 人脸比对结果上传
		 */
	...
	public static class NET_DVR_ALARM_SEARCH_RESULT extends Structure {
		...
		/*
		 * 报警命令，该字段值与报警布防类型相同，目前支持： COMM_VCA_ALARM 0x4993 智能检测报警
		 * COMM_UPLOAD_FACESNAP_RESULT 0x1112 人脸识别结果上传 COMM_SNAP_MATCH_ALARM 0x2902
		 * 人脸比对结果上传
		 */
		public int dwAlarmComm;
		/*
		 * 报警信息，该字段值与报警信息相同，目前支持： 当COMM_VCA_ALARM时，该报警信息为JSON报文
	...
	public static class NET_DVR_SEND_PARAM_IN extends Structure {
		public Pointer pSendData; // 发送的缓冲区,PicURL == 1 的时候，内存中存储的是 URL 字符串,byUploadModeling == 1 的时候，内存中存储的是
									// 建模base64加密数据
		public int dwSendDataLen; // 发送数据长度,PicURL == 1 的时候，表示的 URL 字符串的长度,byUploadModeling == 1
									// 的时候，表示为建模数据base64后的加密长度
		public NET_DVR_TIME_V30 struTime = new NET_DVR_TIME_V30(); // 图片时间
		public byte byPicType; // 图片格式,1-jpg,2-bmp,3-png,4-SWF,5-GIF
		public byte byPicURL; // 图片数据采用URL方式 0-二进制图片数据，1-图片数据走URL方式
		/*
		 * 是否上传建模数据； 0- 二进制图片数据方式(pSendData指向二进制图片数据, dwPicDataLen为图片二进制数据长度)， 1-
		 * 直接上传建模数据(pSendData指向建模base64加密数据, dwPicDataLen为建模数据base64后的加密长度)。
		 * 注：建模数据采用base64加密方式,选择为建模数据上传后，byPicURL 无需。
		 * 当”/ISAPI/Intelligent/channels/<ID>/faceContrast/capabilities”
		 * 能力中返回isSupportUploadModeling能力节点时，支持上传建模数据.
		 */
	...
	public static class NET_DVR_SHIPSDETECTION_ALARM extends Structure {
		...
		public byte byPicTransType; // 图片数据传输方式: 0-二进制；1-url
		public NET_DVR_SHIPSINFO[] struShipInfo = (NET_DVR_SHIPSINFO[]) new NET_DVR_SHIPSINFO()
				.toArray(MAX_SHIPS_NUM); /* 20 */// 船只信息；最大支持20艘
		public int dwPicLen;// 可见光图片长度
		public int dwThermalPicLen;// 热成像图片长度
...
//	public NET_ITC_REDLIGHT_PEDESTRIAN_PARAM struRedLightPed = new NET_ITC_REDLIGHT_PEDESTRIAN_PARAM();//行人闯红灯参数
	...
	public static class NET_DVR_JSON_DATA_CFG extends Structure {
		...
		public Pointer dwInfraredFacePicSize; // 红外人脸图片数据大小，等于0时，代表无人脸图片数据(当JSON报文为当ResponseStatus（JSON）报文时，该字段无意义；当Inbound
												// Data（JSON）报文中没有infraredFaceURL时，该字段需要带上二进制图片内容）
		public byte[] byRes = new byte[248];
	}
	...
	public static class NET_DVR_CAMERAPARAMCFG_EX extends Structure {
		...
		/*
		 * 0-自动光圈, 1-手动光圈, 2-P-Iris1, 3-Union 3-9mm F1.6-2.7 (T5280-PQ1) [IPC5.1.7]
		 * 4-Union 2.8-12mm F1.6-2.7 (T5289-PQ1) [IPC5.1.7] 5-HIK 3.8-16mm
		 * F1.5（HV3816P-8MPIR) 6-HIK 11-40mm F1.7 (HV1140P-8MPIR) 7-HIK 2.7-12mm
		 * F1.2（TV2712P-MPIR） 8- MZ5721D-12MPIR 9- MZ1555D-12MPIR 10-
		...
		public byte byPaletteMode; // 调色板：0-白热，1-黑热，2-调色板2，…，8-调色板8,
									// 9-融合1,10-彩虹,11-融合2,12-铁红1,13-铁红2,14-深褐色,15-色彩1,16-色彩2,17-冰火,18-雨,19-红热,20-绿热,21-深蓝，22-色彩3
		...
		public byte byExposureSegmentEnable; // 0~不启用,1~启用
												// 曝光时间和增益呈阶梯状调整，比如曝光往上调整时，先提高曝光时间到中间值，然后提高增益到中间值，再提高曝光到最大值，最后提高增益到最大值
		public byte byBrightCompensate;// 亮度增强 [0~100]
		/*
		 * 0-关闭、1-640*480@25fps、2-640*480@30ps、3-704*576@25fps、4-704*480@30fps、5-1280*
		 * 720@25fps、6-1280*720@30fps、
		 * 7-1280*720@50fps、8-1280*720@60fps、9-1280*960@15fps、10-1280*960@25fps、11-1280*
		 * 960@30fps、
		 * 12-1280*1024@25fps、13--1280*1024@30fps、14-1600*900@15fps、15-1600*1200@15fps、
		 * 16-1920*1080@15fps、
		 * 17-1920*1080@25fps、18-1920*1080@30fps、19-1920*1080@50fps、20-1920*1080@60fps、
		 * 21-2048*1536@15fps、22-2048*1536@20fps、
		 * 23-2048*1536@24fps、24-2048*1536@25fps、25-2048*1536@30fps、26-2560*2048@25fps、
		 * 27-2560*2048@30fps、
		 * 28-2560*1920@7.5fps、29-3072*2048@25fps、30-3072*2048@30fps、31-2048*1536@12.5、
		 * 32-2560*1920@6.25、
		 * 33-1600*1200@25、34-1600*1200@30、35-1600*1200@12.5、36-1600*900@12.5、37-1280*
		 * 960@12.5fps、38-800*600@25fps、39-800*600@30fps40、
		 * 4000*3000@12.5fps、41-4000*3000@15fps、42-4096*2160@20fps、43-3840*2160@20fps
		 * 、44-960*576@25fps、45-960*480@30fps、46-752*582@25fps、
		 * 47-768*494@30fps、48-2560*1440@25fps、49-2560*1440@30fps
		 * 、50-720P@100fps、51-720P@120fps、52-2048*1536@50fps、53-2048*1536@60fps、
		 * 54-3840*2160@25fps、55-3840*2160@30fps、56-4096*2160@25fps、57-4096*2160@30fps
		 * 、58-1280*1024@50fps、59-1280*1024@60fps、
		 * 60-3072*2048@50fps、61-3072*2048@60fps、62-3072*1728@25fps、63-3072*1728@30fps、
		 * 64-3072*1728@50fps、65-3072*1728@60fps、66-336*256@50fps、67-336*256@60fps、
		 * 68-384*288@50fps、69-384*288@60fps 、70- 640 * 512@50fps 、71- 640 *
		 * 512@60fps、72-2592*1944@25fps、73-2592*1944@30fps、74-2688*1536@25fps、75-2688*
		 * 1536@30fps
		 * 76-2592*1944@20fps、77-2592*1944@15fps、78-2688*1520@20fps、79-2688*1520@15fps、
		 * 80-2688*1520@25fps、81-2688*1520@30fps、82- 2720*2048@25fps、 83-
		 * 2720*2048@30fps、 84-336*256@25fps、85-
		 * 384*288@25fps、86-640*512@25fps、87-1280*960@50fps、88-1280*960@60fps、89-1280*
		 * 960@100fps、90-1280*960@120fps、91-4000*3000@20fps、
		 * 92-1920*1200@25fps、93-1920*1200@30fps、94-2560*1920@25fps、95-2560*1920@20fps、
		 * 96-2560*1920@30fps、97-1280*1920@25fps、98-1280*1920@30fps
		 * 99-4000*3000@24fps、100-4000*3000@25fps、101-4000*3000@10fps、102-
		 * 384*288@30fps、103-2560*1920@15fps、104-2400*3840@25fps、105-1200*1920@25fps
		 * 106-4096*1800@30fps、107-3840*1680@30fps、108-2560*1120@30fps、109-704*320@30fps
		 * 、110-1280*560@30fps、111-4096*1800@25fps、112-3840*1680@25fps
		 * 113-2560*1120@25fps、114-704*320@25fps、115-1280*560@25fps、116-2400*3840@24fps、
		 * 117-3840*2400@24fps、118-3840*2400@25fps、119-2560*1920@12.5fps
		 * 120-2560*2048@12fps、121-2560*2048@15fps、122-2560*1536@25fps、123-2560*1536@
		 * 30fps、124-2256*2048@25fps、125-2256*2048@30fps、126-2592*2592@12.5fps、127-2592*
		 * 2592@15fps、 128 -
		 * 640*512@30fps、129-2048*1520@30fps、130-2048*1520@25fps、131-3840*2160@24fps、132
		 * -2592*1520@25fps、133-2592*1520@30fps、134-2592*1536@25fps、135-2592*1536@30fps
		 * 136-640*960@25fps、137-640*960@24fps、142-2992*2192@25fps、143-2992*2192@30fps、
		 * 144-3008*2160@25fps、145-3008*2160@30fps、146-3072*1728@20fps、147-2560*1440@
		 * 20fps、
		 * 148-2160*3840@25fps、149-2160*3840@30fps、150-7008*1080@25fps、151-7008*1080@
		 * 30fps、152-3072*2048@20fps、153-1536*864@25fps、154-2560*1920@24fps、155-2400*
		 * 3840@30fps、 156-3840*2400@30fps、157-3840*2160@15fps
		 * 158-384*288@8.3fps、159-640*512@8.3fps、160-160*120@8.3fps、161-1024*768@8.3fps、
		 * 162-640*480@8.3fps、163-3840*2160@12.5fps、164-2304*1296@30fps、165-2304*1296@
		 * 25fps、
		 * 166-2560*1440@24fps、167-2688*1512@25fps、168-2688*1512@30fps、169-2688*1512@
		 * 50fps、170-2688*1512@60fps、171-1536*864@30fps、172-2560*1440@50fps、173-2560*
		 * 1440@60fps、
		 * 174-2048*2048@25fps、175-2048*2048@30fps、176-4000*3060@20fps、177-3060*3060@
		 * 25fps、178-3060*3060@30fps、179-3000*3000@25fps、180-3000*3000@30fps、181-8160*
		 * 3616@30fps、
		 * 182-8160*3616@25fps、183-3000*3000@20fps、184-3000*3000@15fps、185-3000*3000@12.
		 * 5fps、186-5472*3648@25fps、187-5472*3648@30fps、188-7680*4320@25fps、189-7680*
		 * 4320@30fps、
		 * 190-8160*2400@25fps、191-8160*2400@30fps、192-5520*2400@25fps、193-5520*2400@
		 * 30fps、194-2560*1440@15fps、195-1944*1212@24fps、196-1944*1212@25fps、197-3456*
		 * 1920@30fps、
		 * 198-4800*2688@25fps、199-4800*2688@30fps、200-6480*1080@25fps、201-6480*1080@
		 * 30fps、202-8640*1440@25fps、203-8640*1440@30fps、204-3456*1920@25fps、205-2688*
		 * 1520@50fps、 206-2688*1520@60fps、207-4976*1452@25fps、208-4976*1452@30fps、
		 * 209-3200*1800@25fps、210-3200*1800@30fps、211-5472*3648@24fps、212-1920*1080@12.
		 * 5fps、213-2944*1656@20fps、
		 * 214-1920*1080@24fps、215-4800*1600@25fps、216-4800*1600@30fps、217-2560*1440@12.
		 * 5fps、218-6560*3690@1fps、219-5120*1400@20fps
		 */
		...
		public byte byFusionMode;// 双光谱视频融合模式，0~热成像模式，1~融合模式，2~画中画模式,3~可见光模式, 4~融合黑白模式,
									// 5~融合彩色模式-草地，6~融合彩色模式-荒地，7~融合彩色模式-雪地，8~融合彩色模式-海洋，9~融合彩色模式-城市
	...
	public static class NET_DVR_WHITEBALANCE extends Structure {
		public byte byWhiteBalanceMode; /*
										 ...
										 * 8-自动跟踪(Auto-Track)9-一次白平衡(One Push)，10-室外自动(Auto-Outdoor)， 11-钠灯自动
										 * (Auto-Sodiumlight)，12-水银灯(Mercury Lamp)，13-自动白平衡(Auto)， 14-白炽灯
										 * (IncandescentLamp)，15-暖光灯(Warm Light Lamp)，16-自然光(Natural Light)
										 */
	...
	public static class NET_DVR_WALLOUTPUTPARAM extends Structure {
		...
		public byte byDisplayMode;/*
									 * 输出连接模式,1-BNC，2-VGA，3-HDMI，4-DVI，5-SDI, 6-FIBER, 7-RGB, 8-YPrPb,
									 * 9-VGA/HDMI/DVI自适应，0xff-无效
									 */
...
//2007-11-30 V211支持以下接口 //11
	...
	public static class NET_DVR_FIREDETECTION_ALARM extends Structure {
		...
		/*
		 * 是否启用PTZ坐标扩展， 0~不启用，PTZ坐标值以wPanPos、wTiltPos、wZoomPos为准。
		 * 1~启用，PTZ坐标值以struPtzPosEx为准。但是新老PTZ都需返回。struPtzPosEx的值需转化为wPanPos、wTiltPos、
		 * wZoomPos值。
		 */
	...
	public static class NET_DVR_CRUISEPOINT_V40 extends Structure {
		public int dwSize;
		public NET_DVR_CRUISEPOINT_PARAM[] struCruisePoint = (NET_DVR_CRUISEPOINT_PARAM[]) new NET_DVR_CRUISEPOINT_PARAM()
				.toArray(128);
		public byte[] byRes = new byte[64];
	}
